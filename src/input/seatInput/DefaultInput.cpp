...
{
    wlr_scene_node* node;
    wl_list_for_each(node, &Core::get().scene->dragIcons->children, link)
    {
...
void DefaultInput::onPointerButton(wlr_pointer_button_event* event)
{
    ...
    if (auto element = Scene::elementFromNode(Core::get().scene->nodeAt(m_seat.getCursor().getPosition(), sCoords)); element)
    {
        // If pressed on a toplevel, focus it
        if (element->type() == SceneElement::TOPLEVEL)
        {
            Core::get().shell->setFocus(static_cast<ToplevelElement*>(element)->getToplevel());
        }
    }
